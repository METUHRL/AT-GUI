...
%			weighting matrix diagonal to use for the reference node. The function
%			should be called as:
%				w = wghFcn(ref);
%			where ref is the reference node being used and w is the
%			corresponding weighting matrix diagonal vector.
%
%		OUTPUT:
%			- cDf - <dim*N,N>double - This is the first order derivative values for
...
%			- cHf - <halfVecDim*N,N>double - This is the second order derivative values for
%			the canonical basis of the function. Each row has the gradients
%			of each node stacked. The input function equals 1 for the
%			node with the same index and 0 for the rest.
%
...
%
%% DEFINE
% check if the geometry is oriented correctly (should be Px3):
...
cDf = zeros(N*dim,N);
halfVecDim = sum(sum(tril(ones(dim,dim)))); % TRIL Extract lower triangular part
